...
<div class="w-full px-6 py-6">
    ...
    {{-- ================= HEADER ================= --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold">Edit Tarif Durasi</h1>
        <p class="text-sm text-gray-500">
            Ubah tarif tambahan berdasarkan durasi
        </p>
    </div>
...
function hitungPreview() {
    const persen = parseFloat(document.getElementById('persentase').value);

    document.querySelectorAll('tbody tr').forEach(row => {
        const tarif = parseFloat(row.querySelector('.tarif-dasar').dataset.tarif);

        if (!isNaN(persen)) {
            const hasil = tarif * persen / 100;
            row.querySelector('.hasil').innerText = 'Rp ' + hasil.toLocaleString('id-ID');
        } else {
...
document.getElementById('persentase').addEventListener('input', hitungPreview);
