...
@section('title', 'Tambah Membership Petugas')

@section('content')
<h1 class="text-2xl font-bold mb-6">Tambah Membership</h1>
...
<form method="POST"
      action="{{ route('petugas.membership.store') }}" 
            ...
            <select name="membership_tier_id"
                ...
                @foreach($tiers as $tier)
                    <option value="{{ $tier->id }}"
                        {{ old('membership_tier_id') == $tier->id ? 'selected' : '' }}>
...
document.querySelector('form').addEventListener('submit', e => {
    ...
    // 1-2 huruf + spasi + 1-4 angka + optional spasi + 0-3 huruf
